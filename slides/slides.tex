...
\begin{frame}{I.2\quad Mapping the baseline onto the theory}
\begin{columns}[T]
\begin{column}{0.55\textwidth}
\textbf{The three policies} (all share the frozen $W_0$):
\begin{itemize}
  \item $\pth$ --- LoRA-adapted policy; the \alert{only} trainable policy (adapters $A,B$)
  \item $\pold$ --- the rollout policy. With $\mu{=}\code{NUM\_ITERATIONS}{=}1$, one gradient step per batch $\Rightarrow$ $\pold{=}\pth$ at the update
  \item $\pref$ --- frozen base Gemma (adapters off); since $B{=}0$ at init, $\pth{=}\pref$ exactly at step 0
\end{itemize}
\end{column}
\begin{column}{0.45\textwidth}
\textbf{Estimator \& clip}
\begin{itemize}
  \item $K{=}\code{NUM\_GENERATIONS}{=}2$; Tunix \code{'grpo'} $=$ \textbf{standard} group-mean, std-normalised over all $K$ (incl.\ $i$) --- \emph{not} leave-one-out
\end{itemize}
\[
  \hat g=\frac1K\sum_i h_i\,\hat A_i,\qquad h_i=\nabla_\theta\log\pth(a_i\mid q)
\]
\begin{itemize}
  \item Clip $\epsilon{=}0.2$, but \alert{inert}: $\mu{=}1\Rightarrow\pold{=}\pth\Rightarrow\rho\equiv1$, nothing is clipped (and it is a \textbf{per-token} ratio, à la DAPO)
\end{itemize}
\end{column}
\end{columns}
\end{frame}

\begin{frame}{I.2\quad The reward --- and why correctness alone stalls}
\textbf{Reward $=$ sum of four} (Tunix rollout reward):
\begin{itemize}
  \item \textbf{Shaping:} \code{match\_format\_exactly}, \code{match\_format\_approximately} (the \code{<reasoning>}/\code{<answer>} template)
  \item \textbf{Correctness:} \code{check\_answer} (gold match, only if the template parses) $+$ \code{check\_numbers} (fallback)
\end{itemize}
\vspace{3pt}
\vspace{2pt}
\textbf{With the correctness reward \emph{alone}, $\sigma_r$ vanishes on most groups} --- not because the model is rarely right (base $=47\%$) but because the reward is \textbf{coarse}: at $K{=}2$ the two siblings usually \emph{tie}, so
\[
  \hat A_i \;=\; \frac{r_i-\bar r}{\sigma_r} \;=\; \frac{0}{\sigma_r+10^{-6}} \;=\; 0 \qquad(\text{exact zero: no signal, not an unstable }0/0).
\]
The dense \textbf{shaping} terms keep $\sigma_r>0$ and push toward parseable outputs --- the precondition for correctness to vary --- but they also invite \alert{reward-hacking} (next).
\end{frame}
