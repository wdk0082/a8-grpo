...
\definecolor{colK}{HTML}{1F6FB2}      % K axis    — prediction (1); neutral, not a verdict
\definecolor{colB}{HTML}{7B3FA0}      % beta axis — prediction (2); neutral, not a verdict
\definecolor{camboxhd}{HTML}{A9C7E8}  % light-blue box header
\definecolor{camboxbd}{HTML}{EDF3FA}  % pale-blue box body
\newcommand{\kcol}[1]{\textcolor{colK}{#1}}
\newcommand{\bcol}[1]{\textcolor{colB}{#1}}
...
\centering
\vspace{2.1cm}
{\color{camblue}\rule{0.92\textwidth}{1.1pt}}\\[14pt]
{\LARGE\bfseries Multi-Agent Systems \& Agentic AI\\[4pt] Practical Examination}\\[14pt]
{\color{camblue}\rule{0.92\textwidth}{1.1pt}}
\vspace{1.7cm}

{\large Zixuan Wang\ \ (zw499)}
\end{frame}
...
{\small Team \textbf{chmawa}:\ \ sm3035\ \ $\cdot$\ \ bc654\ \ $\cdot$\ \ zw499}\\[1.1cm]
...
\begin{frame}{I.1\quad Reproducing the baseline --- the run}
\textbf{(i) The run.}\ \ Baseline reproduced as run \run{R0} (\wb{8c2785ut}) --- upstream \code{borisbolliet/tpu-2026@77c5a67} at its shipped \code{config.py} defaults:
\vspace{6pt}

\begin{center}\small
\renewcommand{\arraystretch}{1.25}
\begin{tabular}{@{}>{\bfseries}l@{\quad}>{\raggedright\arraybackslash}p{0.72\linewidth}@{}}
\toprule
Model & \code{gemma-3-1b-it} (frozen) $+$ LoRA adapters --- rank $64$, $\alpha{=}64$ \\
Algorithm & \textbf{GRPO} --- group-relative advantages $+$ KL penalty to $\pref$ \\
Library & \textbf{Tunix} on \textbf{JAX} \\
Dataset & GSM8K (from HF), train split --- \textbf{1 epoch} $=$ \textbf{3364} GRPO steps \\
GRPO knobs & $\beta{=}0.08$ (KL weight),\ \ $K{=}2$ (responses per prompt),\ \ $\mu{=}1$ (one grad step per rollout batch) \\
Optim.\ \& sampling & LR $3{\times}10^{-6}$ (10\% warmup), grad-clip $0.1$, temperature $0.9$, prompt $\leq256$ / generation $\leq768$ tokens \\
Hardware & TPU \code{v6e-1} \\
Wall-clock & $\sim$\textbf{5\,h\,04\,m} \\
\bottomrule
\end{tabular}
\end{center}
\end{frame}
...
%============================================================
\begin{frame}{I.1\quad Reproducing the baseline --- baseline patches}
...
\begin{itemize}\setlength{\itemsep}{16pt}
  \item TFDS's \code{gsm8k} builder crashes under protobuf 7.x once JAX imports --- we evaluate the \emph{identical} GSM8K test via an HF \code{datasets} fallback (seed 42).
  \item Raised checkpoint retention (\code{SAVE\_INTERVAL\_STEPS}\,$500{\to}100$, \code{MAX\_TO\_KEEP}\,$4{\to}40$) so the \textbf{best-val checkpoint survives selection} (persistence, not training).
\end{itemize}
\vfill
\end{frame}
...
\]
maximal at the baseline's $\kcol{K{=}2}$ --- the noisiest updates, which let the policy chase the dense \textbf{shaping} reward while true correctness decays.
\vspace{1pt}

\textbf{Two predictions:}
\begin{itemize}
  \item[\kcol{\textbf{(1)}}] \kcol{\textbf{Variance.}}\ \ Raising the group size $\kcol{K}$ shrinks $\Var(\hat g)\propto1/\kcol{K}$ $\Rightarrow$ smoother updates, \textbf{milder over-optimisation}.
  \item[\bcol{\textbf{(2)}}] \bcol{\textbf{KL leash.}}\ \ The reference-KL already climbs well above $1$ (previous slide), so a \emph{larger} KL weight $\bcol{\beta}$ (tighter penalty) might rein in the reward-hacking.
\end{itemize}
\vspace{2pt}

\textbf{Test:}\ \ \textbf{5 controlled runs} \run{R\#}$(\kcol{K},\bcol{\beta})$ --- the $\kcol{K}$ rows test \kcol{(1)}, the $\bcol{\beta}$ columns test \bcol{(2)}; only the named knob changes:
\vspace{0pt}
...
\centering
\begin{tabular}{c|ccc}
 & $\bcol{\beta{=}0}$ & $\bcol{\beta{=}0.08}$ & $\bcol{\beta{=}0.3}$ \\ \hline
$\kcol{K{=}2}$ & \run{R4} & \run{R0}\ (baseline) & \run{R1} \\
$\kcol{K{=}8}$ & \run{R3b} & \run{R5} & --- \\
\end{tabular}\par}
\end{frame}
...
\toprule
Model & $(K,\beta)$, step & Acc.\,(\%) & 95\% CI & $\Delta$ base & Wall-clock \\
\midrule
Base \code{gemma-3-1b-it} & --- & 47.4 & 44.7--50.0 & --- & --- \\
R0 (baseline) & $(2,.08)$ best 700 & 46.7 & 44.0--49.4 & $-0.7$\ n.s. & \\
R0 & $(2,.08)$ final 3364 & \bad{6.2} & 4.9--7.5 & \bad{$-41.2^{\dagger}$} & 5h04 \\
R4 & $(2,0)$ best 1300 & 52.8 & 50.1--55.5 & $+5.4^{\ast}$ & \\
R4 & $(2,0)$ final 3364 & 48.6 & 45.9--51.3 & $+1.2$\ n.s. & 4h37 \\
\run{R3b} & $(8,0)$ best 1000 & 55.7 & 53.0--58.2 & $+8.3^{\ast}$ & \\
\good{\run{R3b}} & $(8,0)$ \textbf{final 3364} & \good{\textbf{57.2}} & 54.6--59.9 & \good{$+9.9^{\ast}$} & \textbf{7h08} \\
R5 & $(8,.08)$ best 1300 & 53.4 & 50.7--56.2 & $+6.1^{\ast}$ & \\
R5 & $(8,.08)$ final 3364 & 55.6 & 52.8--58.2 & $+8.2^{\ast}$ & 7h21 \\
\bottomrule
\end{tabular}
\end{center}
\vspace{0pt}
{\footnotesize Wall-clock $=$ \textbf{full-run} cost on \code{v6e-1} (all runs 3364 steps); the best checkpoint is only identifiable \emph{after} the full run. $K{=}8$ buys $+9.9$\,pp for ${\sim}1.4\times$ the compute. \run{R1}\,$(2,.3)$ fell \bad{below base} (best 37.2 / final 0.0).}
\end{frame}

\begin{frame}{I.3\quad Improving the baseline}
\textbf{Held-out accuracy per checkpoint} --- the previous table as a bar chart (exact match, $\pm$95\% bootstrap CI):
\vspace{2pt}
\centering
\includegraphics[width=0.93\linewidth]{F2b_accuracy_table.pdf}\\[4pt]
\raggedright\small
\good{\textbf{R3b}} ($K{=}8$) tops out at \textbf{57.2\%} ($+9.9$\,pp over base); both $K{=}8$ runs beat base at best \emph{and} final, while \bad{R0}'s final checkpoint \bad{collapses} to 6.2\%.
\end{frame}
...
\small
\textbf{The cost of $K{=}8$: group degeneracy.}
\begin{itemize}
  \item About \textbf{a third of groups are degenerate} --- all $K$ rewards equal $\Rightarrow$ \textbf{zero advantage} (wasted samples; the dead-group / effective-sample-size failure mode).
  \item Yet within-group $\sigma_r$ \textbf{holds $\approx1.5$} --- steady, \emph{not} $\to0$.
  \item Net effect still \good{$+8$--$10$\,pp}.
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.45\linewidth}@{\ \ $\rightarrow$\ \ }>{\raggedright\arraybackslash}p{0.45\linewidth}@{}}
\toprule
