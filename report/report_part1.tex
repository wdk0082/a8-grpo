...
\paragraph{(iii-b)}
Under mixture sampling,
\[
    \bar r=\frac1K\sum_{j=1}^K r_j
    \to
    \mu_m=V^{\pi_{\mathrm{mix}}}(q),
\]
not $\mu_p=V^{\pi_{\mathrm{old}}}(q)$. Therefore the unweighted group mean estimates the wrong baseline.
...
\[
\begin{aligned}
    \E_m[w(a)s_\theta(a)A_m(a)]
    &=
    \E_p[s_\theta(a)A_m(a)] \\
    &=
    \E_p[s_\theta(a)A_p(a)]
    +
    (\mu_p-\mu_m)\E_p[s_\theta(a)].
\end{aligned}
\]
Thus
\[
...
At $\theta=\theta_{\mathrm{old}}$, one has the score-function identity
\[
    \E_p[s_{\theta_{\mathrm{old}}}(a)]
    =
    \sum_a p(a)\nabla_\theta\log p(a)
    =
    \sum_a \nabla_\theta p(a)
...
\[
    \boxed{
    \widehat\mu_p^{\mathrm{IS}}
    =
    \frac1K\sum_{j=1}^K w_j r_j.
    }
\]
Indeed,
\[
    \E[\widehat\mu_p^{\mathrm{IS}}]
    =
    \E_m[wR]
    =
    \mu_p.
...
\[
    \boxed{
    \widehat\mu_p^{\mathrm{SN}}
    =
    \frac{\sum_{j=1}^K w_jr_j}{\sum_{j=1}^K w_j}.
    }
\]
...
\[
    \boxed{
    \left(\widehat\sigma_p^{\mathrm{SN}}\right)^2
    =
    \frac{
        \sum_{j=1}^K w_j(r_j-\widehat\mu_p^{\mathrm{SN}})^2
    }{
        \sum_{j=1}^K w_j
    }.
    }
...
\[
    \boxed{
    \widehat g_{\mathrm{mix}}^{\mathrm{corr}}
    =
    \frac1K\sum_{i=1}^K
    w_i s_\theta(a_i)
    \frac{r_i-\widehat\mu_p}{\widehat\sigma_p}.
    }
\]
...
\[
    ...
    \widetilde A_i
    =
    w_i\frac{r_i-\widehat\mu_p}{\widehat\sigma_p},
    \qquad
    \widehat g_{\mathrm{mix}}^{\mathrm{corr}}
    =
    \frac1K\sum_{i=1}^K s_\theta(a_i)\widetilde A_i.
    }
\]
Here $\widehat\mu_p$ may be either the raw estimator $\widehat\mu_p^{\mathrm{IS}}$ or the self-normalized estimator $\widehat\mu_p^{\mathrm{SN}}$. The raw estimator is unbiased for $\mu_p$; the self-normalized estimator is often more stable but is generally not exactly finite-sample unbiased.

For an exactly unbiased finite-sample gradient without empirical standardization, one may use the leave-one-out baseline
\[
    \widehat\mu_{p,-i}^{\mathrm{IS}}
    =
    \frac1{K-1}\sum_{j\neq i}w_jr_j,
\]
giving
\[
    ...
    \frac1K\sum_{i=1}^K
    w_i s_\theta(a_i)
    \left(r_i-\widehat\mu_{p,-i}^{\mathrm{IS}}\right).
    }
\]
Because $\widehat\mu_{p,-i}^{\mathrm{IS}}$ is independent of $a_i$ and has expectation $\mu_p$, this is unbiased for
\[
    \E_p[s_\theta(a)(R(q,a)-\mu_p)].
\]
Empirical standardization by a random $\widehat\sigma_p$ is generally a variance-control heuristic rather than an exactly unbiased operation.
...
\[
    ...
    \alpha\sigma_u^2
    +
    \alpha(1-\alpha)(\mu_p-\mu_u)^2.
    }
\]
Therefore
\[
    \Var_{m_\alpha}(R)-\sigma_p^2
    =
    \alpha\left[\sigma_u^2-\sigma_p^2+(1-\alpha)(\mu_p-\mu_u)^2\right].
\]
